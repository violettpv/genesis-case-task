...
function CoursePage() {
  ...
  const addVideoUrl = (title, url) => {
    setVideoUrl(<LessonVideo url={url} title={title} />);
  };
          ...
          <div className="course-page-box-top">
            <div className="course-page-box-top__left">
              ...
              <p className="course-description">
                <i>Duration:</i> {courseData.duration} minutes
              </p>
              <p className="course-description">
                <i>Rating:</i> {courseData.rating}
              </p>
              <p className="course-description">
                <i>Tags:</i> #{courseData.tags.join(", ")}
              </p>
            </div>
            <div className="course-page-box-right">
              <div className="course-page-box-top__right-img-box">
                <img
                  className="course-page-box-top__right-img"
          ...
          <div className="course-page-box-bottom">
            <div className="course-page-box__video">
              <video width="600" height="auto" controls>
                <source
                  src={`${courseData.meta.courseVideoPreview.link}`}
                  type="video/mp4"
                />
              </video>
              <p>
                  ...
                  {lesson.status === "unlocked" ? (
                    <>
                      <div className="lesson-img-box">
                        <img
                          className="lesson-img-box__img"
                          src={`${lesson.previewImageLink}/${lesson.order}.webp`}
                          alt="lesson"
                          onClick={() => addVideoUrl(lesson.title, lesson.link)}
                        />
                      </div>
                      <h4 className="course-page-box__lessons-card-title">
                        {lesson.title}
                      </h4>
                      <p className="course-page-box__lessons-card-description">
                        {lesson.description}
                      </p>
                      <p className="course-page-box__lessons-card-description">
            ...
            <div className="lesson-video">{videoUrl}</div>
          </div>
        </div>
      </div>
    </div>
